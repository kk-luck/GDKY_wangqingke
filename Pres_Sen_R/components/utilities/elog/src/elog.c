...
ElogErrCode elog_init(void) {
    ...
    /* set Elog assert hook */
    //elog_assert_set_hook(elog_user_assert_hook);

    /* initialize Elog Flash plugin */
    //elog_flash_init();
...
void elog_start(void) {
    ...
    /* show version */
    log_i("Logger is initialize success.");
}
...
/**
 * output RAW format buffer output
 *
 * @param log will be buffered line's log
 * @param size log size
 */
void elog_raw_buff(const char *log, size_t size) {
    size_t log_len = 0;
    ...
    elog_buf_output(log, log_len);
#else
    elog_port_output(log, log_len);
#endif
    /* unlock output */
    elog_output_unlock();
}
