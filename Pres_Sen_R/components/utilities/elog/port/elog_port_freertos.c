...
ElogErrCode elog_port_init(void) {
    ...
    // elog lock
#ifdef ELOG_OS
    output_lock = xSemaphoreCreateCounting(1, 1);
    if (NULL == output_lock) {
    ...
    xTaskCreate(async_output, "elog_tsk", ELOG_TASK_STK, NULL, tskIDLE_PRIORITY, &async_thread);
...
void elog_port_output(const char *log, size_t size) {
    /* output to terminal */
    if (NULL != log && size) {
        if (strncmp("AT+NMGR", log, 7))
        {
          SEGGER_RTT_Write(0, log, size);
          //BusSend(BUS_RS232, (const uint8_t *)log, size);
        }
...
const char *elog_port_get_time(void) {
    ...
    snprintf(cur_system_time, 9, "%02d:%02d:%02d", g_tTime.hour, g_tTime.minute, g_tTime.second);
