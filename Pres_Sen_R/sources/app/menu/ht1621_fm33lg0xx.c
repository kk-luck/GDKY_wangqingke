/**
  ...
  * @version  V1.00
  * @date     17-June-2019
  * @brief    ht1621 lcd driver for n32g45x.
  ******************************************************************************
  */
...
#define GPIO_PIN_CS          FL_GPIO_PIN_0
#define GPIO_PIN_RD          FL_GPIO_PIN_5
#define GPIO_PIN_WR          FL_GPIO_PIN_4
#define GPIO_PIN_DATA        FL_GPIO_PIN_7
#define GPIO_PIN_IRQ         FL_GPIO_PIN_6

#define HT1621_CS_H()        FL_GPIO_SetOutputPin(GPIO_PORT_CS, GPIO_PIN_CS)
#define HT1621_RD_H()        FL_GPIO_SetOutputPin(GPIO_PORT_RD, GPIO_PIN_RD)
#define HT1621_WR_H()        FL_GPIO_SetOutputPin(GPIO_PORT_WR, GPIO_PIN_WR)
#define HT1621_DATA_H()      FL_GPIO_SetOutputPin(GPIO_PORT_DATA, GPIO_PIN_DATA)

#define HT1621_CS_L()        FL_GPIO_ResetOutputPin(GPIO_PORT_CS, GPIO_PIN_CS)
#define HT1621_RD_L()        FL_GPIO_ResetOutputPin(GPIO_PORT_RD, GPIO_PIN_RD)
#define HT1621_WR_L()        FL_GPIO_ResetOutputPin(GPIO_PORT_WR, GPIO_PIN_WR)
#define HT1621_DATA_L()      FL_GPIO_ResetOutputPin(GPIO_PORT_DATA, GPIO_PIN_DATA)

#define HT1621_DATA_READ()   FL_GPIO_GetInputPin(GPIO_PORT_DATA, GPIO_PIN_DATA)
#define HT1621_IRQ_READ()    FL_GPIO_GetInputPin(GPIO_PORT_IRQ, GPIO_PIN_IRQ)
...
static void ht1621_lowlevel_init(void)
{
  ...
  FL_GPIO_Init(GPIO_PORT_CS, &GPIO_InitStructure);
  FL_GPIO_SetOutputPin(GPIO_PORT_CS, GPIO_PIN_CS);

  GPIO_InitStructure.pin = GPIO_PIN_RD;
  FL_GPIO_Init(GPIO_PORT_RD, &GPIO_InitStructure);
  FL_GPIO_SetOutputPin(GPIO_PORT_RD, GPIO_PIN_RD);

  GPIO_InitStructure.pin = GPIO_PIN_WR;
  FL_GPIO_Init(GPIO_PORT_WR, &GPIO_InitStructure);
  FL_GPIO_ResetOutputPin(GPIO_PORT_WR, GPIO_PIN_WR);

  GPIO_InitStructure.pin = GPIO_PIN_DATA;
  FL_GPIO_Init(GPIO_PORT_DATA, &GPIO_InitStructure);
  FL_GPIO_SetOutputPin(GPIO_PORT_DATA, GPIO_PIN_DATA);


  FL_GPIO_SetOutputPin(GPIO_PORT_IRQ, GPIO_PIN_IRQ);
  GPIO_InitStructure.mode = FL_GPIO_MODE_INPUT;
  GPIO_InitStructure.pin = GPIO_PIN_IRQ;
  FL_GPIO_Init(GPIO_PORT_IRQ, &GPIO_InitStructure);
}
...
static uint8_t ht1621_read_bits(uint8_t cnt)
{
  ...
  GPIO_InitStructure.pin = GPIO_PIN_DATA;
  GPIO_InitStructure.mode = FL_GPIO_MODE_INPUT;
	GPIO_InitStructure.pull = FL_ENABLE;
  FL_GPIO_Init(GPIO_PORT_DATA, &GPIO_InitStructure);
