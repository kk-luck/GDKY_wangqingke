...
#define SYSEnterCritical() __disable_irq()  //((void)0U)
#define SYSExitCritical()  __enable_irq()    //((void)0U)  开启总中断​


#ifdef LOG_TAG
	#define log_i(...)  SEGGER_RTT_printf(0,LOG_TAG " [" __TIME__ "] " __VA_ARGS__)
#else
	#define log_i(...)  SEGGER_RTT_printf(0,__FILE__ " [" __TIME__ "] " __VA_ARGS__)
#endif
 ...
 */
//#define SYSGetRunMs()                  (xTaskGetTickCount() * portTICK_PERIOD_MS)
#define SYSGetRunMs()                 (GetMilliSecondCount() * portTICK_PERIOD_MS)
//#define SYSGetRunMsISR()               (xTaskGetTickCountFromISR() * portTICK_PERIOD_MS)
#define SYSGetRunUs()                 (GetMicroSecondCount() * portTICK_PERIOD_US)

//#define SYSGetTimeSec()                TimeGetUnixSecond()
//#define SYSDelayMs(ms)                 vTaskDelay(pdMS_TO_TICKS(ms))


#define SYSGetRunSec         GetSysSecondCount
#define SYSGetTick           GetMilliSecondCount
#define SYSGetTickISR        GetMilliSecondCount
...
#define SYSMalloc(size)                malloc(size)       //pvPortMalloc(size)
#define SYSFree(ptr)                   free(ptr)          //vPortFree(ptr)

//#define SYSEnterCritical()             taskENTER_CRITICAL()
//#define SYSExitCritical()              taskEXIT_CRITICAL()

//#define SYSTaskSuspendAll()            vTaskSuspendAll()
//#define SYSTaskResumeAll()             xTaskResumeAll()
